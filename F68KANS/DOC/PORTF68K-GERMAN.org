...
entsprechenden Vorkehrungen getroffen. So muá ein portables System 
mindestens zwei Anforderungen erfllen:

-	Es muá unabh„ngig von speziellen Ein-/Ausgabegegebenheiten 
sein.
-	Es muá unabh„ngig von speziellen Adreákonfigurationen um 
...
sorgsam im Hauptspeicher verstaut werden. Dazu muá zun„chst 
jeweils ein Puffer fr Code- und Datensegment bereitgestellt 
werden. Bei der Wahl der Gráe dieser Puffer sollte man ein Auge 
darauf haben, was man mit dem Forth-System eigentlich vorhat. Dazu 
ist es wichtig zu wissen, daá F68KANS Code und Daten strikt 
voneinander trennt. Das bedeutet, daá bei Anwendungen, bei denen 
sehr viel Programm compiliert wird, die sich aber nur mit einer 
geringen Menge von Daten befassen, ein groáes Codesegment, bei 
Anwendungen, die viel Daten handhaben oder die auch nur eine Groáe 
Menge von Forth-Worten produzieren (Header sind Daten!), ein 
groáes Datensegment vorzusehen ist (oder eben auch beides).
Fr den ersten Einstieg reichen 64kB Code und 128kB Daten immer.

Wichtig ist natrlich, daá beide Puffer gráer sind, als die 
...
Da man nun alle notwendigen Adressen beisammen hat, muá man dafr 
sorgen, daá F68KANS auch davon erf„hrt. Dazu ist eine Struktur 
...
Die Struktur ist folgendermaáen definiert:
...
Gráen der zur Verfgung gestellten Puffer.
Der letzte Eintrag *si wird erst in den folgenden Kapiteln 
interessant. 

Wenn die Struktur soweit gefllt ist, kann man sagen, daá das 
System geladen ist. Nun muá man daran gehen, es mit einigen 
...
Maáe systemabh„ngig. Deshalb werden die dazu notwendigen 
...
Funktionen gewissen Anforderungen gengen, da sie ja schlieálich 
...
Leistungsmerkmale auáer Acht gelassen habe. Das gibt mir die 
Hoffnung, daá sich die Schnittstelle auch unter anderen Systemen 
so einrichten l„át.
...
Zeichen) vom Stack, der zwingend long sein muá:
...
hnlich wie bei den Speicherbereichen muá auch hier natrlich 
...
besteht darin, daá der Zeiger wiederum auf ein Feld von Zeigern 
...
Fr die Herstellung der Basisfunktion verlangt F68KANS, daá immer 
ein SI Namens 'BIOS' existieren muá und zus„tzlich, daá dieses in 
...
anschlieáend mit den Werten gefllt. Wie man sieht, wird der 
...
kennzeichnet.
Schluáendlich muá nun noch die Adresse der SI in die 
...
durchgefhrt werden.
Anschlieáend werden auf „hnliche Art auch alle Speicherparameter 
entnommen und in die entsprechenden Register bzw. Variablen 
gesetzt. 

5. Nutzung erweiterter Funktionalit„t

Viele Forth-Systeme leiden darunter, daá sie im wesentlichen in 
...
Forth zu erschlieáen. Von dieser Mglichkeit wurde auch von mir 
bei der Implementation von vielen Systemfunktionen (File, Float, 
...) ausgiebig Gebrauch gemacht, so daá F68KANS eines der wenigen 
...
Dazu muá man sich die bentigten Funktionen beschaffen und mit der 
...
Im Gegensatz zu 'BIOS' muá aber nun auch auf Forth-Seite ein 
...
sein. Es wird folgendermaáen benutzt, um den Grundstein fr die 
...
Ein- und Ausgabeparameter zu unterscheiden. Standardm„áig sind 
...
(Zu diesem Beispiel ist zu bemerken, daá 'FILE*', eigentlich ein 
Zeiger, auf Forth-Seite wie eine Integerzahl behandelt wird.) 

Die Funktion '_fputs' ist dann z.B. folgendermaáen einzusetzen:

	S" Hallo Welt"  FILEID @  _fputs   . 


Die Konstante CLIBBASE wird danach nicht weiter verwendet, so daá 
man auf ihre Definition sogar verzichten kann. 

Ich denke, daá diese Beispiele die Benutzung der Schnittstelle mit 
...
achten, daá die Reihenfolge der Funktionen in der SI auf Lader- 
und auf Forth-Seite identisch sein mssen. 

Bevor man nun diese Definitionen wirklich benutzen kann, ist es 
wichtig, daá das mit 'systeminterface' definierte 
...
Auch hier muá zur Laufzeit des Systems das Initialisierungswort 
