F68KANS errohandling
====================


The errorhandling of F68KANS follows the specification of the ANSI
standard for the Forth programming language. 

This handling is based on the words CATCH and THROW and a complete set
of predefined errornumbers. CATCH and THROW are defined in the EXCEPTION
wordset of the standard. In the CORE wordset the more traditional
words ABORT and ABORT" are defined.

	ABORT	( i*x -- )( R: j*x -- )
	ABORT"	Compilation: ( "ccc<quote>" -- )
			Execution: ( i*x xj --  | i*x )( R: j*x --  | j*x )

The ANSI document advises the use of the EXCEPTION words to implement
CATCH and THROW. And this is exactly the way it is done in F68KANS.
